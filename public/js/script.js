$(document).ready(function() {
    var excelFile = document.body.getAttribute('data-excelfile');
    ...
    var stateMapping = {};    // key: state code (col B) → state name (col E) for state rows.
    var districtMapping = {}; // key: district code (col C) → district name (col E) for district rows.

    // Function to initialize DataTable with 20 rows per page.
    ...
    // Function to populate the table based on area type and district filter.
    function populateTable(filterTypes) {
        ...
        // If only Subdistrict is selected and district filter is visible, apply further filtering.
    ...
    // Function to populate the district filter drop-down with unique district names from subdistrict rows.
    ...
    // onload handler to process the Excel file.
    oReq.onload = function(e) {
        // Check if the file was successfully loaded.
        if (oReq.status !== 200) {
            $("#loader").fadeOut();
            $("#resultTable").after('<div class="alert alert-danger" role="alert">Error: Excel file not found. Please try again later.</div>');
            return;
        }
        ...
        for (var area in results) {
            ...
            // Build a sorted array of language details.
            var languagesData = [];
            sortedLang.forEach(function(lang) {
                var cnt = languages[lang];
                var pct = totalPopulation ? ((cnt / totalPopulation) * 100).toFixed(2) : "0.00";
                languagesData.push({ language: lang, count: cnt, percentage: pct });
            });
        ...
        // Trigger change event so district filter appears.
    ...
    oReq.onerror = function(e) {
        $("#loader").fadeOut();
        $("#resultTable").after('<div class="alert alert-danger" role="alert">Network error: Unable to load the Excel file. Please try again later.</div>');
    };

    oReq.send();

    // When an area name is clicked, show the modal with details and languages.
    $(document).on("click", ".area-info", function(e) {
        ...
        // Build the languages table in the modal.
        var modalTableBody = "";
        item.languagesData.forEach(function(langDetail) {
            modalTableBody += "<tr><td>" + langDetail.language + "</td><td>" + langDetail.count + "</td><td>" + langDetail.percentage + "%</td></tr>";
        });
        $("#modalLanguagesTable tbody").html(modalTableBody);
